<x-layout>
    ...
    <main>
        <div class="h3 my-5 text-2xl font-semibold">Perpus SMK Infokom > category > {{ $category->name }}</div>
        {{-- paginate --}}
        <div class="my-5">
            {{ $books->links() }}
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 mb-10 mx-5 mt-10 ">
            @forelse ($books as $book)
                <a href="view/{{ $book->slug }}">
                    <div
                        class="bg-white rounded-lg p-2 flex flex-col justify-between h-full relative shadow-md hover:shadow-2xl transition duration-300">
                        @if ($book->cover)
                            <img src="{{ asset('storage/' . $book->cover) }}" class="shadow-xl rounded-lg mb-3"
                                alt="Cover">
                        @else
